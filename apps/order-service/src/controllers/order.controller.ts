...
)=>
{
   try{
      ...
      const sellerData = shops.map((shop) => ({
         shopId: shop.id,
         sellerStripeAccountId: shop?.sellers?.stripeId,
      }));
      ...
      await redis.setex(`payment_session:${sessionId}`,600,
...
export const createOrder = async(
   ...
   try
   {
      ...
      if(event.type==='payment_intent.succeeded')
      {
         ...
         if(!sessionData){
            console.warn(`Session data not found for sessionId: ${sessionId}`);
...
export const getSellerOrders = async(
   ...
   try
   {
      ...
      //fetch all orders for the seller's shop
